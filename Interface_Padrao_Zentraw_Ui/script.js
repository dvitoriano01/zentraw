...
// --- CONTROLE PLAY/PAUSE E STOP ---
let audioControlsDiv = document.querySelector('.audio-controls');
if (!audioControlsDiv) {
  audioControlsDiv = document.createElement('div');
  audioControlsDiv.className = 'audio-controls';
  // Insere logo após o botão de upload
  const fileBtn = document.getElementById('file-btn');
  if (fileBtn && fileBtn.parentNode) {
    fileBtn.parentNode.insertBefore(audioControlsDiv, fileBtn.nextSibling);
  } else {
...
playPauseBtn.className = 'audio-action-btn';
playPauseBtn.innerHTML = '▶ PLAY';

// Botão Stop
const stopBtn = document.createElement('button');
stopBtn.id = 'stop-btn';
stopBtn.className = 'audio-action-btn';
...
// Remover quebras de linha entre os botões, se houver
Array.from(audioControlsDiv.childNodes).forEach((node) => {
  if (node.nodeType === Node.TEXT_NODE && /\s*/.test(node.textContent)) {
    audioControlsDiv.removeChild(node);
  }
});
// --- FIM DOS CONTROLES ---
  ...
  // ============================================================================
  // ZENTRAW CINEMA RENDER SYSTEM - INTEGRATED
  // ============================================================================
  
  // Cinema render configuration
  const CINEMA_CONFIG = {
    WIDTH: 2560,
    HEIGHT: 1440,
    FPS: 30,
  ...
  let cinemaRenderer = null;
  let isRecording = false;
  let recordingFrameCount = 0;
  let capturer = null;
  
  // Cinema Renderer Class
  class ZentrawCinemaRenderer {
    constructor() {
      this.frameCount = 0;
      ...
      this.startTime = 0;
      console.log('🎬 Cinema Renderer initialized');
    }
    
    setResolution(width, height) {
      CINEMA_CONFIG.WIDTH = width;
      CINEMA_CONFIG.HEIGHT = height;
    }
    
    setDuration(seconds) {
      CINEMA_CONFIG.DURATION_SECONDS = seconds;
    }
    
    setFPS(fps) {
      CINEMA_CONFIG.FPS = fps;
    }
    
    async startRender(audioSrc = './sample_audio2.wav') {
      if (this.isRendering) {
        console.warn('⚠️ Render already in progress');
        return;
      }
      
      console.log('🎬 Starting cinema render...');
      ...
      // Setup cinema canvas
      this.setupCinemaCanvas();
      
      // Initialize CCapture if available
      if (typeof CCapture !== 'undefined') {
        this.capturer = new CCapture({
          format: CINEMA_CONFIG.FORMAT,
          framerate: CINEMA_CONFIG.FPS,
          name: `zentraw-cinema-${Date.now()}`,
          quality: CINEMA_CONFIG.QUALITY,
          verbose: true,
          display: true
        });
        ...
        console.warn('⚠️ CCapture.js not available');
      }
      
      // Load and play audio
      if (currentAudioSrc || audioSrc) {
        const audioToUse = currentAudioSrc || audioSrc;
        console.log('🎵 Loading audio for render:', audioToUse);
        
        if (currentAudioElement) {
          currentAudioElement.src = audioToUse;
          currentAudioElement.currentTime = 0;
          
          currentAudioElement.onloadeddata = () => {
            currentAudioElement.play();
            this.renderLoop();
          };
        ...
        // Start render without audio
        this.renderLoop();
        
        // Auto-stop after duration
        setTimeout(() => {
          this.stopRender();
        }, CINEMA_CONFIG.DURATION_SECONDS * 1000);
      }
      
      // Update UI
      document.getElementById('recording-indicator').style.display = 'block';
      document.getElementById('terminal-status').textContent = 'RENDERING';
      document.getElementById('render-status').textContent = 'Cinema render in progress...';
    }
    
    setupCinemaCanvas() {
      // Store original canvas
      this.originalCanvas = renderer.domElement;
      
      // Resize renderer for cinema quality
      renderer.setSize(CINEMA_CONFIG.WIDTH, CINEMA_CONFIG.HEIGHT);
      renderer.setPixelRatio(CINEMA_CONFIG.PIXEL_RATIO);
      
      // Update camera aspect ratio
      camera.aspect = CINEMA_CONFIG.WIDTH / CINEMA_CONFIG.HEIGHT;
      camera.updateProjectionMatrix();
      
      console.log(`📹 Cinema canvas setup: ${CINEMA_CONFIG.WIDTH}x${CINEMA_CONFIG.HEIGHT}`);
    }
    
    renderLoop() {
      if (!this.isRendering) return;
      
      const time = this.frameCount / CINEMA_CONFIG.FPS;
      const totalFrames = CINEMA_CONFIG.DURATION_SECONDS * CINEMA_CONFIG.FPS;
      
      // Cinema camera movement
      this.updateCinemaCamera(time);
      
      // Render frame
      renderer.render(scene, camera);
      
      // Capture frame
      if (this.capturer && this.frameCount < totalFrames) {
        this.capturer.capture(renderer.domElement);
      }
      
      // Update progress
      const progress = Math.round((this.frameCount / totalFrames) * 100);
      if (this.frameCount % (CINEMA_CONFIG.FPS * 5) === 0) { // Every 5 seconds
        console.log(`🎬 Render progress: ${progress}% (${this.frameCount}/${totalFrames} frames)`);
        document.getElementById('render-status').textContent = `Rendering: ${progress}% complete`;
    ...
    updateCinemaCamera(time) {
      // Cinematic camera movements
      const slowTime = time * 0.1;
      
      // Orbital movement
      const radius = 7 + Math.sin(slowTime * 0.5) * 1;
      const height = Math.sin(slowTime * 0.3) * 2;
      const angle = slowTime * 0.2;
    ...
    stopRender() {
      console.log('🏁 Stopping cinema render...');
      ...
      if (this.capturer) {
        this.capturer.stop();
        this.capturer.save();
        console.log('💾 Cinema render saved successfully');
      }
      ...
      document.getElementById('render-status').textContent = 'Cinema render completed!';
      
      // Add terminal message
      addTerminalMessage('CINEMA RENDER COMPLETE. OUTPUT SAVED.', true);
      showNotification('CINEMA RENDER COMPLETED');
      
      console.log('✅ Cinema render completed successfully');
    }
  ...
  // Initialize cinema renderer
  if (typeof CCapture !== 'undefined') {
    cinemaRenderer = new ZentrawCinemaRenderer();
    console.log('✅ Cinema render system ready');
  } else {
    console.warn('⚠️ CCapture.js not loaded. Cinema rendering not available.');
  }
  
  // Connect cinema render button
  const renderBtn = document.getElementById('render-btn');
  if (renderBtn) {
    renderBtn.addEventListener('click', () => {
      if (cinemaRenderer && !cinemaRenderer.isRendering) {
        // Get settings from UI
        const resolutionSelect = document.getElementById('resolution-select');
        const durationSlider = document.getElementById('duration-slider');
        const fpsSelect = document.getElementById('fps-select');
        
        if (resolutionSelect && durationSlider && fpsSelect) {
          ...
          cinemaRenderer.setResolution(width, height);
          cinemaRenderer.setDuration(duration);
          cinemaRenderer.setFPS(fps);
          
          // Start cinema render
          cinemaRenderer.startRender();
      ...
      } else if (!cinemaRenderer) {
        console.warn('⚠️ Cinema renderer not available');
        alert('Cinema renderer not available. Please ensure CCapture.js is loaded.');
      } else {
        console.warn('⚠️ Render already in progress');
        alert('Render already in progress. Please wait for completion.');
      }
    });
  }
  ...
  if (previewBtn) {
    previewBtn.addEventListener('click', () => {
      ...
      // Just play the current audio and show the visualization
      if (currentAudioElement && currentAudioElement.src) {
        currentAudioElement.currentTime = 0;
        currentAudioElement.play();
        
        addTerminalMessage('PREVIEW MODE ACTIVATED.', true);
        showNotification('PREVIEW MODE ACTIVE');
      } else {
