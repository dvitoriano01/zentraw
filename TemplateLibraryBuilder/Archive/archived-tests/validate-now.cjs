...
function testEndpoints() {
  ...
  try {
    const healthResult = execSync('curl -s http://localhost:5000/health', { encoding: 'utf-8', timeout: 5000 });
  ...
  try {
    const blenderResult = execSync('curl -s http://localhost:5000/api/blender/test', { encoding: 'utf-8', timeout: 5000 });
