...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('render_log_v1.4.0.a.9.1.log'),
...
def main():
    """Função principal."""
    logger.info("🛡️ Iniciando Zentraw 3D Visualizer V1.4.0.a.9.1")
    try:
        # Obtém argumentos da linha de comando
        argv = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else []
        if len(argv) < 3:
            logger.error("Uso: blender template.blend --background --python render_audio_visualizer_v1.4.0.a.9.1.py -- audio_path image_path output_path [params_json]")
            sys.exit(1)
